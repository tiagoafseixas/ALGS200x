...
public class FibonacciService {
    ...
    static class FibonacciHelper {
        ...
        static final BigDecimal getPartialSumofFibonacci(final long start, final long end) {

            final BigDecimal lowerEnd = getFibonacciValue(start + 1).subtract(BigDecimal.ONE);
            final BigDecimal upperEnd = getSumofFibonacci(end);
            final BigDecimal diff = upperEnd.subtract(lowerEnd);
    ...
    public static void main(String[] args) {
        ...
        final long newN = n % period;
        final long newM = m % period;

        System.out.println(FibonacciHelper.getPartialSumofFibonacci(newN, newM).remainder(BIGDECIMALTEN));
    }
}